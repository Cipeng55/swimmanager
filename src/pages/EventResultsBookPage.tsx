...
const EventResultsBookPage: React.FC = () => {
  ...
  const processDataForBook = useCallback((
    ...
    groupedByRace.forEach((raceEntries, raceKey) => {
      ...
      let rankCounter = 0;
      let lastTimeMs = -1;
      let swimmersAtLastRank = 1; // How many swimmers were at the previous rank level
      ...
      timedEntries.forEach((entry) => {
          ...
          if (!isSP) {
              ...
              if (currentTimeMs > lastTimeMs) {
                  // New rank level, jump the counter by the number of swimmers at the previous rank
              ...
              entry.rank = undefined; // SP swimmers don't get a rank number.
          }
      });
