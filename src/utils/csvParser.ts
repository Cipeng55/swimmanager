 ...
 * Expects headers roughly matching: Name,DOB,Gender,Club,GradeLevel (case-insensitive for GradeLevel)
 * @param csvText The raw CSV string.
 * @returns An array of ParsedSwimmerRow objects.
 * @throws Error if essential headers (Name, DOB, Gender, Club) are missing.
 */
export const parseSwimmerCsv = (csvText: string): ParsedSwimmerRow[] => {
  ...
  const canonicalExpectedHeaders: { [key: string]: keyof ParsedSwimmerRow } = {
    ...
    gender: 'Gender',
    club: 'Club',
    gradelevel: 'GradeLevel', // Added, expecting "GradeLevel" or similar in CSV
  };
  const requiredDisplayHeaders: (keyof ParsedSwimmerRow)[] = ['Name', 'DOB', 'Gender', 'Club'];
  ...
  rawHeaders.forEach(rawHeader => {
    const lowerRawHeader = rawHeader.toLowerCase().replace(/\s+/g, ''); // Normalize header: lowercase, remove spaces
  ...
  if (missingRequiredDisplayHeaders.length > 0) {
    throw new Error(`CSV Headers are missing or incorrect. Missing required headers similar to: ${missingRequiredDisplayHeaders.join(', ')}. Please ensure your CSV has columns for Name, DOB, Gender, and Club. GradeLevel is optional.`);
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Initialize all canonical fields to undefined for consistent object shape
