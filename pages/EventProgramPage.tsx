...
const EventProgramPage: React.FC = () => {
  ...
  const handlePrintProgram = () => {
    ...
    const racesWithHeatsData = numberedUniqueRaces.map(race => ({
      race: race,
    ...
    const printData: EventProgramPrintData = {
      event, 
      numberedUniqueRaces, 
      racesWithHeats: racesWithHeatsData
    };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {numberedUniqueRaces.length === 0 && !loading && (
      ...
      {numberedUniqueRaces.map((race, index) => {
        const heats = getHeatsForRace(race);
